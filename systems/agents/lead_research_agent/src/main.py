...
from src.models.schemas import LeadResearchRequest, LeadResearchAnalysisResult, HealthResponse, ApiClientPublic, AccountResponse, AdminCreateClientRequest, AdminCreateClientResponse, CreditTopupRequest
...
app = FastAPI(title="Lead Research Agent", version=settings.APP_VERSION, lifespan=lifespan)
...
async def health():
    return HealthResponse(status="healthy", service="lead-research-agent", version=settings.APP_VERSION)
...
@app.post("/api/v1/leads/analyze")
@limiter.limit("10/minute")
async def analyze(request: Request, payload: LeadResearchRequest, client=Depends(require_client)):
    try:
