...
from src.models.schemas import MonitoringRequest, MonitoringAnalysisResult, HealthResponse, ApiClientPublic, AccountResponse, AdminCreateClientRequest, AdminCreateClientResponse, CreditTopupRequest
...
app = FastAPI(title="Tender Monitoring Agent", version=settings.APP_VERSION, lifespan=lifespan)
...
async def health():
    return HealthResponse(status="healthy", service="tender-monitoring-agent", version=settings.APP_VERSION)
...
@app.post("/api/v1/monitoring/analyze")
@limiter.limit("10/minute")
async def analyze(request: Request, payload: MonitoringRequest, client=Depends(require_client)):
    try:
    ...
    title = ", ".join(payload.target_sectors[:2])
    db.save_analysis(client["id"], result.id, f"Monitor: {title}", result.dict())
    out = result.dict()
