...
from src.models.schemas import ContentRequest, ContentAnalysisResult, HealthResponse, ApiClientPublic, AccountResponse, AdminCreateClientRequest, AdminCreateClientResponse, CreditTopupRequest
...
app = FastAPI(title="Social Content Ops Agent", version=settings.APP_VERSION, lifespan=lifespan)
...
async def health():
    return HealthResponse(status="healthy", service="social-content-ops-agent", version=settings.APP_VERSION)
...
async def analyze(request: Request, payload: ContentRequest, client=Depends(require_client)):
    try:
    ...
    db.save_analysis(client["id"], result.id, payload.brand_name, result.dict())
    out = result.dict()
