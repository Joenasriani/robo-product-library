...
from src.models.schemas import VideoRequest, VideoAnalysisResult, HealthResponse, ApiClientPublic, AccountResponse, AdminCreateClientRequest, AdminCreateClientResponse, CreditTopupRequest
...
app = FastAPI(title="Robot Demo Video Generator", version=settings.APP_VERSION, lifespan=lifespan)
...
async def health():
    return HealthResponse(status="healthy", service="robot-demo-video-generator", version=settings.APP_VERSION)
...
async def analyze(request: Request, payload: VideoRequest, client=Depends(require_client)):
    try:
    ...
    db.save_analysis(client["id"], result.id, payload.robot_name, result.dict())
    out = result.dict()
    out["credits_remaining"] = updated["credits_remaining"]
    return out

@app.get("/api/v1/video/history")
