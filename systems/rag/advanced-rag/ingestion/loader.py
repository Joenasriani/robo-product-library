"""ingestion.loader — advanced chunking with sentence-level boundary awareness.

Supports PDF, TXT, MD. Uses smaller chunks (600 chars, 120 overlap) for
better precision in cross-encoder reranking.
"""
...
def ingest(
    ...
    """Load and chunk a document with sentence-aware splitting."""
    source = Path(source)
...
def _chunk(docs, filename, chunk_size, chunk_overlap):
