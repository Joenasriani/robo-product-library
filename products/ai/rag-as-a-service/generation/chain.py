"""generation.chain — LangChain RAG chain for rag_as_a_service."""
...
def generate(
    ...
    return Answer(
        ...
        confidence=confidence, retrieved_chunks=len(context),
    )
...
def _format_context(context):
    ...
    for i, (doc, score) in enumerate(context, start=1):
        ...
        parts.append(f"[{i}] Source: {source} ({loc}) | Relevance: {score:.2f}\n{doc.page_content}")
    return "\n\n---\n\n".join(parts)
