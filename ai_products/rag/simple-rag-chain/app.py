...
# ── Path setup: allow importing from this product folder ─────────────────────
...
with left_col:
        ...
        if all_chunks:
            with st.spinner(f"Indexing {len(all_chunks)} chunks into Chroma…"):
                ...
                try:
                    ...
                    _set_status("success", f"INDEXED — {len(all_chunks)} chunks")
                except Exception as exc:
                    _set_error(f"Indexing failed: {exc}\n\n" + traceback.format_exc())
                    st.rerun()
    ...
    if st.session_state["store"] is None and store_exists(store_id_input.strip()):
        st.markdown(
            ...
            f"<code>{store_id_input.strip()}</code> found on disk.</small>",
            unsafe_allow_html=True,
        )
    ...
    if ask_btn:
        ...
        with st.spinner("Generating answer…"):
            try:
                ...
                _set_error(f"Generation failed: {exc}\n\n" + traceback.format_exc())
                st.rerun()
