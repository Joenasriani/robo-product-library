...
def save_analysis(client_id, request_id, request_title, analysis_dict):
    conn = _connect()
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO analysis_history (client_id, request_id, request_title, analysis_json, credits_used, created_at) VALUES (?, ?, ?, ?, 1, ?)",
        (client_id, str(request_id), str(request_title)[:200], _json.dumps(analysis_dict), datetime.utcnow().isoformat())
    )
    conn.commit()
    conn.close()
...
def get_history(client_id, limit=50):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT id, request_id, request_title, analysis_json, created_at FROM analysis_history WHERE client_id = ? ORDER BY id DESC LIMIT ?",
        (client_id, limit)
    )
    rows = cur.fetchall()
    conn.close()
