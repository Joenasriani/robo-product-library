...
from src.models.schemas import RFPRequest, RFPAnalysisResult, HealthResponse, ApiClientPublic, AccountResponse, AdminCreateClientRequest, AdminCreateClientResponse, CreditTopupRequest
...
app = FastAPI(title="Enterprise RFP Response Assistant", version=settings.APP_VERSION, lifespan=lifespan)
...
async def health():
    return HealthResponse(status="healthy", service="enterprise-rfp-response-assistant", version=settings.APP_VERSION)
...
async def analyze(request: Request, payload: RFPRequest, client=Depends(require_client)):
    try:
    ...
    db.save_analysis(client["id"], payload.id, f"RFP: {payload.company_name}", result.dict())
    out = result.dict()
...
@limiter.limit("10/minute")
async def analyze_file(request: Request, file: UploadFile = File(...), company_name: str = Form(...), company_capabilities: str = Form(...), submission_deadline: str = Form(""), client=Depends(require_client)):
    ...
    rfp = RFPRequest(
        id=f"upload-{abs(hash(file.filename)) % 1000000}",
        rfp_text=extracted[:15000],
    ...
    try:
        ...
        raise HTTPException(status_code=402, detail="Insufficient credits")
    try:
        result = await run_analysis(rfp)
    except Exception as exc:
        logger.error("LLM failed: %s", exc, exc_info=True)
        raise HTTPException(status_code=502, detail="LLM analysis failed.")
    db.save_analysis(client["id"], rfp.id, f"RFP: {company_name}", result.dict())
    out = result.dict()
    out["credits_remaining"] = updated["credits_remaining"]
    out["extracted_characters"] = len(extracted)
    return out

@app.get("/api/v1/rfp/history")
