...
def init_db():
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS protocol_products (
        ...
        description TEXT NOT NULL, price_aed REAL NOT NULL, status TEXT NOT NULL DEFAULT "inquiry_only",
        ...
        delivery_type TEXT NOT NULL DEFAULT "protocol_configuration_bundle", created_at TEXT NOT NULL)""")
...
def load_canonical_products(conn):
    """Load protocol products from canonical manifests (idempotent by slug).

    Reads the manifest index from settings.CATALOG_INDEX, then loads each
    referenced manifest.yaml relative to settings.REPO_ROOT.
    Products already present in the DB (matched by slug) are not duplicated.
    Only products with status 'inquiry_only' are loaded; non-commercial entries
    are skipped.
    """
    try:
        import yaml
    ...
    manifest_paths = index.get("products", [])
    cur = conn.cursor()
    for rel_path in manifest_paths:
        ...
        if not slug:
            continue
        if manifest.get("status") == "non_commercial" or manifest.get("sales_mode") == "non_commercial":
            continue
        cur.execute("SELECT id FROM protocol_products WHERE slug = ?", (slug,))
        if cur.fetchone():
            continue
        countries = manifest.get("supported_countries", [])
        region = ", ".join(countries) if countries else "GCC"
        cur.execute("""INSERT INTO protocol_products (
                        slug, name, category, region, description, price_aed, status, version, risk_level,
                        hardware_requirements_json, included_files_json, use_cases_json, limitations_json,
                        support_mode, delivery_type, created_at
                    ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                    (
                        slug,
                        ...
                        float(manifest.get("price_aed", 0)),
                        manifest.get("status", "inquiry_only"),
                        manifest.get("version", "1.0.0"),
                        manifest.get("risk_level", "medium"),
                        json.dumps(manifest.get("hardware_requirements", [])),
                        json.dumps(manifest.get("deliverables", [])),
                        json.dumps(manifest.get("supported_use_cases") or manifest.get("best_for", [])),
                        json.dumps(manifest.get("limitations", [])),
                        manifest.get("fulfillment_method", "manual_delivery_after_inquiry_approval"),
                        manifest.get("deliverable_type", "protocol_configuration_bundle"),
...
def list_products(active_only=True):
    conn = _connect()
    cur = conn.cursor()
    q = "SELECT * FROM protocol_products WHERE status = 'inquiry_only' ORDER BY id" if active_only else "SELECT * FROM protocol_products ORDER BY id"
    cur.execute(q)
    rows = cur.fetchall()
    conn.close()
