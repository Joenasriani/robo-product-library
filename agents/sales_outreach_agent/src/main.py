...
app = FastAPI(title="Sales Outreach Agent", version=settings.APP_VERSION, lifespan=lifespan)
...
async def health():
    return HealthResponse(status="healthy", service="sales-outreach-agent", version=settings.APP_VERSION)
...
@app.post("/api/v1/outreach/analyze")
@limiter.limit("10/minute")
async def analyze(request: Request, payload: OutreachRequest, client=Depends(require_client)):
    try:
